...
var startupDesktopModePromise = applyWallpaperModeState(false);
Promise.resolve(startupDesktopModePromise).then(function () {
  if (typeof setDesktopLock === 'function') return setDesktopLock(fx.desktopLock === true, true);
  return false;
}).catch(function () { });
...
initIdleGuideCanvas();
var startupLoginStatusPromise = Promise.all([refreshLoginStatus(), refreshQQLoginStatus({ forceVip: true, reason: 'startup' }), refreshKugouLoginStatus()]);
