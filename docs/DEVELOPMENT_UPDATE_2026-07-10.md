# Mineradio v1.1.4 开发更新与提交审计

## 本次开发内容

- 网易云、QQ 音乐、酷狗三平台账号状态同时保留，界面默认聚焦当前平台，“全部”仅用于汇总。
- 酷狗新增二维码、网页和 Cookie 登录，接入账号、歌单、歌单详情、搜索、播放地址、歌词与音质选择。
- 搜索新增 `KG`，`All` 聚合三平台并优先排列当前账号平台结果。
- 本地音乐库支持选择并恢复文件夹、文件夹二级歌单、同目录封面/歌词、本地搜索和默认 MR 节奏分析。
- 支持导入 QQ、网易云、酷我、酷狗和咪咕公开歌单，并在本地保存。
...
- Electron 正式运行时，网易云、QQ、酷狗会话文件位于 Electron `userData` 目录。
- 直接运行 `server.js` 时，项目根目录可能生成 `.cookie`、`.qq-cookie` 或 `.kugou-cookie`。
- `.kugou-cookie` 用于保存酷狗会话和设备标识；即使当前文件只有设备标识，也按私有数据处理。
- 上述 Cookie 文件均已由 `.gitignore` 排除，不应提交、上传或粘贴到 Issue。
- 酷狗签名常量是客户端协议兼容参数，不是用户账号凭据；用户 Cookie、token 和二维码登录结果不会写入源码。
