...
- 小云、小狗账号状态同时保留，界面默认聚焦当前平台，“全部”仅用于汇总。
...
- Electron 正式运行时，小云、小狗会话文件位于 Electron `userData` 目录。
- 直接运行 `server.js` 时，项目根目录可能生成 `.cookie` 或 `.kugou-cookie`。
