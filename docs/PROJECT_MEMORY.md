...
- 2026-07-06 当前本机代理端口改为 `127.0.0.1:7890`；GitHub CLI / `gh auth refresh` / Release 上传 / electron-builder 下载构建资源需要代理时优先使用 `127.0.0.1:7890`。不要走旧代理 `127.0.0.1:26001`，该端口会 `connection refused`。需要临时修复时先清空 `HTTP_PROXY`/`HTTPS_PROXY`，再设为 `http://127.0.0.1:7890`。
...
- 禁止回退或改坏的点：不要把真实代码目录改回旧外层源码目录；不要忘记当前 GitHub / 打包下载代理端口 `127.0.0.1:7890`。
