...
- 登录小Q后顶部能显示小Q账号状态，但很快又显示未登录，或者只有账号状态没有小Q头像和昵称。
- 小Q歌单能读取，但点击小Q正式歌曲听不了。
- `/api/qq/song/url` 对部分片段/非正式曲目可返回 URL，但正式曲目常返回 `104003`。

## 已确认根因

- `p_skey` 只能证明网页小Q账号态，不等于小Q播放授权。
- 真正更接近播放授权的 cookie 字段是 `qm_keyst`、`qqmusic_key`、`music_key`，微信通道可看 `wxskey`。
- 只有 `p_skey` 时，资料、歌单可能可用，但正式歌曲 vkey 仍可能不给 `purl`。
- 小Q资料接口 `fcg_get_profile_homepage.fcg` 可能返回 `code:1000`，此时不能直接把账号判成未登录，应从 cookie 的 `ptnick_*` 和 `qlogo.cn` 兜底昵称/头像。

## 当前修复点

- `server.js`
  - `qqCookieNickname()` 从 `ptnick_*` 等 cookie 兜底小Q昵称。
...
- `desktop/main.js`
  - 小Q登录窗口不再一拿到 `p_skey` 就自动关闭。
  - 登录后会继续跳到小Q播放器页 warmup，等待 `qm_keyst`、`qqmusic_key`、`music_key` 或 `wxskey`。
  - 如果用户手动关窗但只有网页态，返回 `partial: true`。
- `public/index.html`
  - 小Q登录状态只有在 `profileUnavailable` 且没有昵称/头像时才标记 stale。
  - 小Q播放失败先降音质重试，再自动查找小云同名同歌手版本换源。
...
4. 如果 `playbackKeyReady=false` 且 `qqCode=104003`，优先重新跑小Q官方登录窗口，不要先改搜索或播放器 audio 逻辑。
